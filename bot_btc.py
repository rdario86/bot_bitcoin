...
st.set_page_config(page_title="BOT Estrategia ORB (Dual) - Bitcoin", layout="wide")

# ==========================================
# 1. PARÁMETROS DE LA ESTRATEGIA Y CAPITAL
# ==========================================
with st.sidebar.form(key='panel_ajustes'):
    st.header("⚙️ Ajustes ORB (Dual 15m)")
...
# ==========================================
# 3. MOTOR DE BACKTESTING (AMERICANA + ASIÁTICA)
# ==========================================
def ejecutar_backtest(df, ratio, capital_inicial, riesgo_pct):
    ...
    for fecha in fechas:
        ...
        # ===============================================
        # 1. SESIÓN AMERICANA (09:30 a 16:00)
        # ===============================================
        ...
        if not vela_us.empty:
            ...
            for idx, row in horario_us.iterrows():
                ...
                if tipo_trade:
                    ...
                    operaciones.append({
                        'Mercado': 'Americana 🇺🇸', 'Fecha': idx, 'Tipo': tipo_trade, 'Entrada': entrada,
        ...
        # ===============================================
        # 2. SESIÓN ASIÁTICA (20:00 a 02:00)
        # ===============================================
        vela_as = df_calc.loc[(df_calc['Date'] == fecha) & (df_calc.index.time == pd.to_datetime('20:00').time())]
        if not vela_as.empty:
            max_orb = vela_as['High'].iloc[0]
            min_orb = vela_as['Low'].iloc[0]
            
            horario_as = df_calc.loc[(df_calc['Date'] == fecha) & (df_calc.index.time >= pd.to_datetime('20:15').time()) & (df_calc.index.time <= pd.to_datetime('23:45').time())]
            
            for idx, row in horario_as.iterrows():
                entrada = row['Close']
                tamano = row['Tamaño_Vela']
                prom = row['Promedio_Tamaño_10']
                tipo_trade = None
                ...
                if tipo_trade:
                    ...
                    riesgo_usd = capital_actual * (riesgo_pct / 100)
                    df_post = df_calc.loc[idx:]
                    # El cierre Asiático es a las 02:00 del día siguiente
                    limite_tiempo = idx.replace(hour=2, minute=0, second=0) + pd.Timedelta(days=1)
                    ...
                    for jdx, vela in df_post.iterrows():
                        ...
                        if jdx >= limite_tiempo:
                            ...
                            dist = (precio_cierre - entrada) if "Long" in tipo_trade else (entrada - precio_cierre)
                            pnl_usd = (dist / riesgo_precio) * riesgo_usd
                            resultado = "Ganancia (02:00) ⏱️✅" if pnl_usd > 0 else "Pérdida (02:00) ⏱️❌"
                            break
                    ...
                    capital_actual += pnl_usd
                    operaciones.append({
                        'Mercado': 'Asiática 🇯🇵', 'Fecha': idx, 'Tipo': tipo_trade, 'Entrada': entrada,
                        'Stop Loss': stop_loss, 'Take Profit': take_profit, 'Resultado': resultado,
                        'Max_ORB': max_orb, 'Min_ORB': min_orb, 'Vela_Ruptura': tamano, 'Promedio_10_Velas': prom,
                        'PnL ($)': pnl_usd, 'Balance': capital_actual
                    })
...
# ==========================================
# 4. INTERFAZ Y RESULTADOS (CON FILTROS DE MERCADO)
# ==========================================
st.title("📈 BOT Estrategia ORB Dual (EEUU + Asia)")
...
if df_btc.empty:
    ...
    st.markdown("### 🌍 Selecciona el Mercado a Analizar")
    filtro_mercado = st.radio(
        "Filtro:",
        ["Global (Ambos Mercados) 🌐", "Solo Americana (09:30 NY) 🇺🇸", "Solo Asiática (20:00 NY) 🇯🇵"],
        horizontal=True,
        label_visibility="collapsed"
    )
    
    # Aplicar Filtro
    if "Americana" in filtro_mercado:
        df_filtrado = df_operaciones[df_operaciones['Mercado'] == 'Americana 🇺🇸']
    elif "Asiática" in filtro_mercado:
        df_filtrado = df_operaciones[df_operaciones['Mercado'] == 'Asiática 🇯🇵']
    else:
        df_filtrado = df_operaciones

    if df_filtrado.empty:
        st.warning("No hay operaciones para el mercado seleccionado en este rango.")
    else:
        # División de datos filtrados
        df_regulares = df_filtrado[~df_filtrado['Resultado'].str.contains("16:00|02:00")]
        df_tiempo = df_filtrado[df_filtrado['Resultado'].str.contains("16:00|02:00")]

        total_trades = len(df_filtrado)
        balance_final_filtrado = capital_inicial + df_filtrado['PnL ($)'].sum()
        ganancia_neta = df_filtrado['PnL ($)'].sum()
        rentabilidad = (ganancia_neta / capital_inicial) * 100

        st.subheader(f"📊 Resumen de Rendimiento - {filtro_mercado}")
        
        tab1, tab2, tab3 = st.tabs(["Totales (Suma)", "Cierres por SL/TP", "Cierres Forzados"])

        with tab1: # TOTALES
            aciertos_tot = len(df_filtrado[df_filtrado['Resultado'].str.contains("Ganancia")])
            fallos_tot = len(df_filtrado[df_filtrado['Resultado'].str.contains("Pérdida")])
            ...
            c6.metric("Balance Proyectado", f"${balance_final_filtrado:,.2f}")
        ...
        columnas_finales = ['Mercado', 'Fecha', 'Tipo', 'Entrada', 'Stop Loss', 'Take Profit', 'Expansión', 'Resultado', 'PnL ($)', 'Balance']
        ...
        opciones_trades = df_filtrado['Fecha'].dt.strftime('%Y-%m-%d %H:%M:%S').tolist()
        trade_seleccionado = st.selectbox("Selecciona la fecha del Trade:", opciones_trades)
        
        if trade_seleccionado:
            trade_data = df_filtrado[df_filtrado['Fecha'].dt.strftime('%Y-%m-%d %H:%M:%S') == trade_seleccionado].iloc[0]
            fecha_obj = trade_data['Fecha']
            dia_str = fecha_obj.strftime('%Y-%m-%d')
            
            # Ajuste dinámico de los gráficos según el mercado (la sesión Asiática cruza la medianoche)
            if "Americana" in trade_data['Mercado']:
                df_dia = df_btc.loc[f"{dia_str} 08:30:00":f"{dia_str} 16:30:00"]
            else:
                dia_siguiente = (fecha_obj + pd.Timedelta(days=1)).strftime('%Y-%m-%d')
                df_dia = df_btc.loc[f"{dia_str} 19:30:00":f"{dia_siguiente} 02:30:00"]
            ...
            fig.update_layout(
                title=f"Trade {trade_data['Tipo']} ({trade_data['Mercado']}) el {dia_str} | Resultado: {trade_data['Resultado']}",
